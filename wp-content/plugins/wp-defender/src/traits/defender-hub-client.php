...
trait Defender_Hub_Client {
	...
	private function hub_api_request(
		...
		if ( ! $dash_api->has_key() ) {
			throw new Exception( esc_html__( 'Dash plugin authentication API key missing.', 'wpdef' ) );
		}
	...
	public function can_wpmu_free_request() {
		if ( ! class_exists( 'WPMUDEV_Dashboard' ) ) {
			throw new Exception( esc_html__( 'WPMU DEV Dashboard plugin is missing.', 'wpdef' ) );
		}
